...
import { bytesToNumberLE, bytesToNumberBE } from '@noble/curves/utils.js'
...
export function getPublicKeyNobleEd25519(certificate) {
	...
	return module.Point.fromHex(xBuffer.toString('hex')).toBytes()
}
...
export function getPublicKeyNobleSecp256k1(certificate) {
	...
	return module.Point.fromAffine({ x, y }).toBytes()
	// ALTERNATIVE: return module.Point.fromHex('04' + xBuffer.toString('hex') + yBuffer.toString('hex')).toBytes()
}
...
export function demoCertificates() {
	...
	if (module == secp256k1) {
		...
		let signature2DER = module.Signature.fromBytes(signature2).toBytes('der')
		...
		let verificationResult2CNodeCrypto = crypto.verify(null, Buffer.from("test"), certificate.publicKey, module.Signature.fromBytes(signature2).toBytes('der'))
		console.log("Verification noble-certificate_nodecrypto (secp256k1): ", verificationResult2CNodeCrypto)

		let signaturePoint = module.Signature.fromBytes(signature, 'der')
		let verificationResult12 = module.verify(signaturePoint.toBytes(), encodedMessage, publicKey2, { prehash: true, lowS: false })
		console.log("Verification node-noble (secp256k1): ", verificationResult12)
	}
}